...
define('uit/require/PluginInterface', function () {
    'use strict';
...
define('uit', ['uit/require/PluginInterface', 'uit/uitags/uiTags', 'jscore/handlebars/handlebars'], function (PluginInterface, uit, Handlebars) {
    ...
    return PluginInterface.extend({
        ...
        load: function (fileName, req, onLoad, config) {
            ...
            fileName = this.parseName(fileName);
            this.fetch(req.toUrl(fileName), function (content) {
                this.parse(fileName, content, config, function (content) {
                    onLoad(content);
                }, req);
            }.bind(this), function (err) {
                onLoad.error(err);
        ...
        parse: function (name, content, config, cb, req) {

            if (config.isBuild) {
                cb(content);
            } else {
                uit.process(content, function (data) {
                    // if the content has handlebars then pass the template if not just pass null
                    var handlebarsTemplate = (data.handlebars === true) ? template.parse(name, data.html, config) : null;
                    uit.parseWithDependencies(content, handlebarsTemplate, function (view) {
                        cb(view);
                    }.bind(this), req);
                }.bind(this), req);
            }
        ...
        writeFile: function (pluginName, fileName, req, write, config) {
            ...
            this.load(fileName, req, function (content) {
                this.write(pluginName + "!" + fileName, content, config, req, function (obj) {
                    req(obj.dependencies, function () {
                    });
                    write.asModule(pluginName + "!" + fileName, fileName, obj.module);
                }, fileName);

            }.bind(this), config);
        },

        write: function (name, content, config, req, callback, fileName) {
            // Process html
            uit.process(content, function (data) {
                // if the path is relative use toUrl to ge full path, then remove the base directory
                var ds = data.dependencies.map(function (dep) {
                    var exclamationMarkIndex = dep.indexOf('!');
                    if (dep[0] === '.' || (exclamationMarkIndex !== -1 && dep[exclamationMarkIndex + 1] === '.')) {
                        if (exclamationMarkIndex > -1) {
                            var pluginName = dep.substring(0, exclamationMarkIndex);
                            var fileName = req.toUrl(dep.substring(exclamationMarkIndex + 1)).substr(config.baseUrl.length + 1);
                            return pluginName + "!" + fileName;
                        } else {
                            return req.toUrl(dep).substr(config.baseUrl.length + 1);
                        }
                    } else {
                        return dep;
                    }
                ...
                var additionalDeps = ['jscore/core', 'uit/uitags/uiTags'];
                // add handlebar if needed
                if (data.handlebars === true) {
                    additionalDeps.unshift('jscore/handlebars/handlebars');
                }

                var deps = additionalDeps.concat(ds);
                var originalDeps = additionalDeps.concat(data.dependencies);

                // paths are replaced with shorter strings to get a smaller file size
                data.dependencies.forEach(function (dep, index) {

                    // the custom implementation relays on the path, so no substitution for layouts
                    if (dep.indexOf('layouts/') !== -1) {
                        return;
                    }

                    var replacement = String(index);

                    // replace all the occurrences of the path in the html
                    content = content.replace(new RegExp(dep, 'g'), replacement);

                    // if present in the originalDeps array, replace
                    if (originalDeps.indexOf(dep) !== -1) {
                        originalDeps[originalDeps.indexOf(dep)] = replacement;
                    }
                    // if present in the styles array, replace
                    if (data.styles.indexOf(dep) !== -1) {
                        data.styles[data.styles.indexOf(dep)] = replacement;
                    }

                });

                // just an styles array
                var styleArray = (data.styles.length > 0) ? '["' + data.styles.join('","') + '"]' : '[]';

                // we don't want handlebars in the dependencyArray so if it is there we remove it
                var dependencyArray = (data.handlebars === true) ? '["' + originalDeps.slice(1).join('","') + '"]' : '["' + originalDeps.join('","') + '"]';

                // if handlebars is used, parse template, if not, the content string will be the actual html, single quotes are used to wrap it so they need to be escaped
                var contentString = (data.handlebars === true) ? 'Handlebars.template(' + template.parse(name, content, config) + ')' : '\'' + content.replace(new RegExp("'", 'g'), "\\\'") + '\'';

                // if we take the arguments from the beginning or not (because we don't want to handlebars)
                var depIndex = (data.handlebars === true) ? '+1' : '';

                // import handlebars or not
                var handlebarsImport = (data.handlebars === true) ? 'Handlebars, ' : '';


                var module = 'define(["' + deps.join('","') + '"],function (' + handlebarsImport + 'core, uit) { ' +
                    'var dependencies = {};' +
                    'var deps = ' + dependencyArray + ';' +
                    'for(var i = 0; i< deps.length; i++){ ' +
                    'dependencies[deps[i]] = arguments[i' + depIndex + '];' +
                    '} ' +
                    'var styles = ' + styleArray + '; ' +
                    'return uit.parse(' + contentString + ', dependencies, styles);});' + "\n";
